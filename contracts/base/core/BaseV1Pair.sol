...
contract BaseV1Pair is IERC20, IPair, ReentrancyGuard {
    ...
    uint256 internal constant SWAP_FEE_VOLATILE = 1000;
    ...
    function setIsPriorityPair(bool ifPriority) external {
        require(msg.sender == factory);
        isPriorityPair = ifPriority;

        if (isPriorityPair == true) {
            if (stable) {
                ///@dev 0.0075% stable swap fee
                swapFee = 75;
            } else {
                ///@dev 0.05% volatile swap fee
                swapFee = 500;
            }
        } else {
            if (stable) {
    ...
    function claimPartnerFee() external returns (uint256 partnerClaimed0, uint256 partnerClaimed1) {
        ...
        if (partnerClaimed0 > 0 || partnerClaimed1 > 0) {
            partnerClaimable0 = 0;
            partnerClaimable1 = 0;
            uint256 addressLength = partnerAddresses.length;

            for (uint256 i = 0; i < addressLength; i++) {
                BaseV1Fees(fees).claimFeesFor(
                    partnerAddresses[i],
                    (partnerClaimed0 / addressLength),
                    (partnerClaimed1 / addressLength)
                );
            }
        }
    }
