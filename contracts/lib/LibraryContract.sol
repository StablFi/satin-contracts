...
contract satin_library is Initializable {
    satin_router internal router;

    // constructor(address _router) {
    //     router = satin_router(_router);
    // }
    ...
    function getTradeDiff(
        ...
        (uint256 dec0, uint256 dec1, uint256 r0, uint256 r1, bool st, address t0, ) = satin_pair(
    ...
    function getMinimumValue(
        ...
        (uint256 dec0, uint256 dec1, uint256 r0, uint256 r1, , address t0, ) = satin_pair(
            router.pairFor(tokenIn, tokenOut, stable)
        ).metadata();
        uint256 sample = tokenIn == t0 ? (r0 * dec1) / r1 : (r1 * dec0) / r0;
        return (sample, r0, r1);
